...
window.onload = function(){
	...
	btnCheckUid.onclick = function(){
		...
		// 아이디 중복 체크
		fetch('/farmstory/user/checkUser.do?type=uid&value='+uid)
			.then(resp=>resp.json())
			.then(data=> {
				if(data.result > 0){
	...
	registerForm.nick.addEventListener('focusout', function(){
		...
		fetch('/farmstory/user/checkUser.do?type=nick&value='+nick)
			.then(resp=>resp.json())
			.then(data=>{
				console.log(data);
				if(data.result > 0){
	...
	btnSendEmail.onclick = async function(){
		...
		try {
			...
			const response = await fetch('/farmstory/user/checkUser.do?type=email&value='+email);
	...
	btnAuthEmail.onclick = function(){
		...
		fetch('/farmstory/user/checkUser.do', {
			method: 'POST',
	...
	registerForm.hp.addEventListener('focusout', async function(){
		...
		try {
			const response = await fetch('/farmstory/user/checkUser.do?type=hp&value='+hp);
			const data = await response.json();
